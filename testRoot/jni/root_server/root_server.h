...
<head>
    ...
    <script>
		...
		function initAppListModal() {
			...
			var filterAppInput = document.getElementById('filterAppInput');
			var clearInput = document.getElementById('myAppListClear');
			var listAppItems = document.getElementById('listAppItems');
			...
			filterAppInput.oninput = function() {
				var filterValue = this.value.toLowerCase();
				var items = listAppItems.getElementsByClassName('list-item');
				...
				if (filterValue) {
					...
					filterAppInput.style.width = 'calc(100% - 40px)';
				} else {
					clearInput.style.display = 'none';
					filterAppInput.style.width = '100%';
				}
			}

			clearInput.onclick = function() {
				filterAppInput.value = '';
				filterAppInput.dispatchEvent(new Event('input'));
			}
        ...
        function installSuBtnClick() {
			...
			.then(data => {
				...
				if(data.err === '0') {
					...
					const message = "安装部署su成功，su路径已复制到剪贴板。\n\n请注意，本次安装的su仅当天有效，请在过期后卸载并重新安装部署su。如需移除此过期限制，您可自行修改源码。";
					alert(message);
				}
			})
			.catch(error => {
		...
		function injectSuInApp(appName) {
			let jsonData = {
				type: 'injectSu',
				name: appName
			};
		...
		function addAppListModalItem(txt) {
			var listAppItems = document.getElementById('listAppItems');
			...
			div.onclick = function(event) {
				...
				hideAppListModal();
				injectSuInApp(clickName);
			}
			listAppItems.appendChild(div);
		}

		function cleanAppListModal() {
			var listAppItems = document.getElementById('listAppItems');
			while (listAppItems.firstChild) {
				listAppItems.removeChild(listAppItems.firstChild);
			}
		}
		...
		function getAppList() {
			...
			.then(data => {
				for(let i = 0; i < data.content.length; i++) {
					addAppListModalItem(data.content[i]);
				}

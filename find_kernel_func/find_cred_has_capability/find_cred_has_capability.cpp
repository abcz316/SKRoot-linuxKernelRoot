...
void SearchFeature1(char* image, size_t size) {
	...
	for (int y = 0; y < 8; y++) {
		char* pAddress = FindBytes(image, size, &feature4[y * 4], 4);
		while (pAddress) {
			partInfo info;
			...
			pAddress += 4;
			pAddress = FindBytes(pAddress, size - ((size_t)pAddress - (size_t)image), &feature4[y * 4], 4);
		}
...
'\xFE', '\x03', '\x1E', '\x32', 
	...
	for (int i = 0; i < vSearch6.size(); i++) {
		partInfo info = vSearch6.at(i);
		for (size_t s = 0; s < sizeof(info.partHex); s += 4) {
			if (memcmp((void*)((size_t)info.partHex + (size_t)s), (BYTE*)&feature7[0 * 4], 4) == 0) {
